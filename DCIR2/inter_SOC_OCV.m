...
load('HNE_FCC_06_DCIR2_026.mat') % DCIR2 큰 pulse에 해당하는 data 추출
...
SOC_inv2 = zeros(size(V_restC));

% Perform extrapolation for the first element
SOC_inv2(1) = interp1(OCV_unique, SOC_unique, V_restC(1), 'linear', 'extrap');

% Perform interpolation for the remaining elements
for i = 2:length(BigIC)
    SOC_inv2(i) = interp1(OCV_unique, SOC_unique, V_restC(i));
end
