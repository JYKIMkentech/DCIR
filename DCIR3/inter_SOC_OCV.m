...
SOC_inv3 = zeros(size(V_restC));

% Perform extrapolation for the first element
SOC_inv3(1) = interp1(OCV_unique, SOC_unique, V_restC(1), 'linear', 'extrap');

% Perform interpolation for the remaining elements
for i = 2:length(BigIC)
    SOC_inv3(i) = interp1(OCV_unique, SOC_unique, V_restC(i));
end
